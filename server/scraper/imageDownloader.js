...
const imageDownloader = async () => {
  await mongoose.connect('mongodb+srv://afikn:g9DdFeyb4SKZz4Vx@cluster0.xjawb8j.mongodb.net/Effi');
  ...
  for (let product of products) {
    ...
    for (let imageURL of product.images) {
      const imageName = imageURL.split('/').filter((pathName) => pathName.indexOf('.jpg') !== -1);
    ...
    await ProductModel.findOneAndUpdate({ _id: new ObjectId(product._id) }, product);
  }
};
